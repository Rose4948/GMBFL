...
class NlEncoder( nn.Module ):
    def __init__(self, args):
        ...
        self.transformerBlocks = nn.ModuleList(
            [TransformerBlock( self.embedding_size, 1, self.feed_forward_hidden, 0.1 ) for _ in range(args.layer_size)] )
        self.token_embedding = nn.Embedding( args.Nl_Vocsize, self.embedding_size - 1 )
        self.token_embedding1 = nn.Embedding( args.Nl_Vocsize, self.embedding_size - 4 )
    ...
    def forward(self, input_node, inputtype, inputad, res, inputtext, linenode, linetype, linemus, LineSMType):
